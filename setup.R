...
linegraph_sharks <- function(colour1 = "darkorange", colour2 = "purple", colour3 = "olivedrab3", colour4 = "firebrick3")
{
  plotme <- data %>%
    filter(year > 2009) %>%
    count(species, year) %>%
    complete(species, year, fill = list(n = 0))
  linegraph1 <<- ggplot(plotme) +
    geom_line(aes(x = year, y = n, col = species)) +
    geom_point(aes(x = year, y = n, col = species)) +
    scale_colour_manual(values = c(colour1, colour2, colour3, colour4)) +
    theme_bw(12) + ylab("Count") + xlab("Year") +
    theme(legend.position = "none") +
